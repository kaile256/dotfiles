...
let s:weedBufnames = ['fugitive:\/\/\/']
function! s:nobuffers(bufnr) abort "{{{1
  let buftype = getbufvar(a:bufnr, &bt)
...
command! GonlyDiffStage
      \ :Gw | only | call s:staging_HEAD_diff()
command! -bar -bang -nargs=* -complete=customlist,fugitive#EditComplete GonlyDiff
...
nnoremap <silent> <space>go     :<c-u>GonlyDiffStage<cr>
