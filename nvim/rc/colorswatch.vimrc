...
cnoreabbr <silent><expr> palette   (getcmdtype() == ':' && getcmdline() =~ '^palette$')?   'ColorSwatchGenerate cterm csv <cr> :setl ft=qf bt=quickfix' : 'palette'
cnoreabbr <silent><expr> pale   (getcmdtype() == ':' && getcmdline() =~ '^pale$')?   'ColorSwatchGenerate cterm csv <cr> :setl ft=qf bt=quickfix' : 'pale'
cnoreabbr <silent><expr> pal   (getcmdtype() == ':' && getcmdline() =~ '^pal$')?   'ColorSwatchGenerate cterm csv <cr> :setl ft=qf bt=quickfix' : 'pal'
